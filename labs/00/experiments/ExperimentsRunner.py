...
class ExperimentsRunner:
    ...
    def save_read_me(self):
        output = [
            f"{DELIMITER_README}{str(alg)}{DELIMITER_README}"
            for alg in EXPERIMENT_CONSTANTS.ALGORITHMS_TO_RUN
        ]
        output_algorithms = f"\n".join(output)
        read_me = f"{READ_ME_HEADER}\n{output_algorithms}"
        self.save_to_file(read_me, "README", ".txt")

    def start_experiments_for_functions(
        self, functions=EXPERIMENT_CONSTANTS.FUNCTION_TO_RUN
    ):
        self.results = {}
        start = time.time()
    ...
    def save_to_file(
        self, text, name, suffix="-experiments.csv", directory="experiments"
    ):
        self.check_save()
